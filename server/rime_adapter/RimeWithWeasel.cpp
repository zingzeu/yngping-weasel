...
#include "common/StringAlgorithm.hpp"
#include "include/WeaselUtility.h"
#include "common/WeaselVersion.h"
#include <VersionHelpers.h>

#include "include/rime/rime_api.h"
...
// Hardcoded UI style
void _FakeUIStyle(weasel::UI* ui);
...
void RimeWithWeaselHandler::_Setup()
{
	...
	printf("Share data dir: %s\n", weasel_traits.shared_data_dir);
	weasel_traits.user_data_dir = weasel_user_data_dir();
	printf("user data dir: %s\n", weasel_traits.user_data_dir);
	...
	weasel_traits.app_name = "rime.yngpingweasel";
...
{
	printf("RimeWithWeasel: Initialize\n");
	...
	printf("\tRimeWithWeasel: Before load config\n");
	RimeConfig config = { NULL };
	if (RimeConfigOpen("weasel", &config))
	{
		printf("\tRimeWithWeasel: Config open\n");
		if (m_ui)
		{
			//_UpdateUIStyle(&config, m_ui, true);
			_FakeUIStyle(m_ui);
			m_base_style = m_ui->style();
			printf("\tRimeWithWeasel: UI config loaded\n");
			printf("\tRimeWithWeasel: UI font: %ls, font_size: %d\n", m_base_style.font_face.c_str(), m_base_style.font_point);
		}
...
UINT RimeWithWeaselHandler::FindSession(UINT session_id)
{
	...
	printf("Find session: session_id = %u, found = %s\n", session_id, found ? "True" : "False");
...
UINT RimeWithWeaselHandler::AddSession(LPWSTR buffer, EatLine eat)
{
	if (m_disabled)
	{
		printf("Trying to resume service.\n");
		EndMaintenance();
		if (m_disabled) return 0;
	}
	UINT session_id = RimeCreateSession();
	printf("Add session: created session_id = %u\n", session_id);
...
UINT RimeWithWeaselHandler::RemoveSession(UINT session_id)
{
	if (m_ui) m_ui->Hide();
	if (m_disabled) return 0;
	printf("Remove session: session_id = %u\n", session_id);
...
{
	std::cout << "Process key event: keycode = " << keyEvent.keycode << ", mask = " << keyEvent.mask
...
{
	std::cout << "Commit composition: session_id = " << session_id<<std::endl;
...
{
	std::cout << "Clear composition: session_id = " << session_id <<std::endl;
	if (m_disabled) return;
...
{
	std::cout << "Focus in: session_id = " << session_id << ", client_caps = " << client_caps <<std::endl;
	if (m_disabled) return;
	_UpdateUI(session_id);
	m_active_session = session_id;
}
...
{
	std::cout << "Focus out: session_id = " << session_id << std::endl;
	if (m_ui) m_ui->Hide();
	m_active_session = 0;
}
...
static void _FakeUIStyle(weasel::UI* ui)
{
	if (!ui) return;
	...
	style.margin_y = 12;
	style.spacing = 10;
	style.candidate_spacing = 5;
	style.hilite_spacing = 4;
	style.hilite_padding = 2;
	style.round_corner = 4;
	// color scheme

	style.text_color = 0x3a3942;
	style.candidate_text_color = 0x222126;
	style.back_color = 0xfcfcfc;
	style.border_color = 0xebebeb;
	style.hilited_text_color = 0xf2f2f2;
	style.hilited_back_color = 0x393935;
	style.hilited_candidate_text_color = 0xffffff;
	style.hilited_candidate_back_color = 0xe35b1c;
	style.label_text_color = blend_colors(style.candidate_text_color, style.back_color);
	style.hilited_label_text_color = blend_colors(style.hilited_candidate_text_color, style.hilited_candidate_back_color);
	style.comment_text_color = 0x222126;
	style.hilited_comment_text_color = 0xffffff;
}
